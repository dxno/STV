...
# ==========================================
# 1. DATABASE SETUP (V4 Schema - Multi-Question)
# ==========================================
...
if action_param in ["vote", "results"] and election_id_param:
    ...
    deadline_dt = datetime.datetime.strptime(election['deadline'], "%Y-%m-%d %H:%M:%S")
    ...
    if action_param == "vote":
        ...
        for q in questions_data:
            st.divider()
            st.markdown(f"## {q['title']}")
            # Extract only the ballots relevant to this specific question
...
# FETCH FULL ELECTIONS DATA FRAME FOR ALL TABS
    ...
    if elections_df.empty:
        ...
        with col1:
            ...
            if st.button("Authorize Batch Voters"):
                ...
                for v in voters:
                    c.execute("INSERT OR IGNORE INTO voter_status (election_id, voter_hash, is_allowed, has_voted) VALUES (?, ?, 1, 0)", (int(v_id), hash_identifier(v)))
                conn.commit()
        ...
        with col2:
            ...
            if st.button("Generate & Authorize IDs"):
                ...
                for nid in new_ids:
                    c.execute("INSERT OR IGNORE INTO voter_status (election_id, voter_hash, is_allowed, has_voted) VALUES (?, ?, 1, 0)", (int(v_id), hash_identifier(nid)))
                conn.commit()
...
with sub_turnout:
    if not elections_df.empty:
        ...
        # FIX 1: Explicitly get the ID and cast to int to ensure DB compatibility
